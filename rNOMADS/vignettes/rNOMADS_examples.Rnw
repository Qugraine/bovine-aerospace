...
\title{Downloading and Analyzing Weather Forecast Data with rNOMADS}
...
The package consists of two parts: a downloader that gathers data from the NOMADS website, and a reader that loads the model data into R.
...
Note that not all operating systems are supported - Windows users probably have to use Cygwin.

The rNOMADS package currently accesses the Global Forecast System 0.5 x 0.5 degree model.
...
It produces forecasts for every 3 hours out to 192 hours, after which it switches to every 12 hours.
Future versions of rNOMADS will access more forecast models (for example, the North American Mesoscale).
...
model.date = Sys.time() #Get the latest model run
fcst.date = Sys.time() #Get the forecast as close to now as possible

#Download model file
file.name <- GribGrab(levels, variables, 
    model.date = model.date, fcst.date = fcst.date, verbose = FALSE)

#Read the model file
grb.data <- ReadGrib(file.name, variables, levels)

#Make an array for quick indexing
lon.grid <- 0.5
lat.grid <- 0.5
atmos <- ModelGrid(grb.data, lon.grid, lat.grid)
@
...
model.date <- Sys.time()
fcst.date <- model.date + 3600 * 6


file.name <- GribGrab(levels, variables, 
    model.date = model.date, fcst.date = fcst.date,
    verbose = FALSE)
...
We can generate atmospheric profiles for temperature, wind speed, and sound speed by selecting variables and levels for a small region of interest.
In this case, we will investigate the temperature and wind speed versus height at a model grid node near Chapel Hill, North Carolina ($36^{\circ}$ N, $79^{\circ}$ W).
The tropopause is clearly visible in the temperature profile at around 15,000 m (Figure \ref{fig:temp_profile}).
Wind speeds are surprisingly high in the upper atmosphere, exceeding 150 km/hr at 50,000 m (Figure \ref{fig:wind_profile}).
We end with a quick calculation of sound velocity versus height (Figure \ref{fig:c_profile}) using the formula:
\begin{equation}
...
file.name <- GribGrab(levels, variables, 
    model.domain = model.domain, verbose = FALSE)
...
#Choose which model grid node to use
loni <- which(atmos$x == -79)
lati <- which(atmos$y == 36)

#Get height in meters
hgti <- which(atmos$variables == "HGT")
hgt <- atmos$z[,hgti,loni,lati]
...
vi <- which(atmos$variables == "TMP")
tmp <- atmos$z[,vi, loni, lati] - 273.15
...
wui <- which(atmos$variables == "UGRD")
wvi <- which(atmos$variables == "VGRD")

wu <- atmos$z[, wui, loni, lati]
wv <- atmos$z[, wvi, loni, lati]
wvel <- sqrt(wu^2 + wv^2) 
