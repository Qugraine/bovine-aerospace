...
The rNOMADS package provides access to 54 models.
...
urls.out <- CrawlModels(abbrev = "gfs0.5", 
...
#Download model file
file.name <- GribGrab(urls.out[1], latest.pred, 
    levels, variables, verbose = FALSE)

#Read the model file
grb.data <- ReadGrib(file.name, levels, variables)

#Make an array for quick indexing
atmos <- ModelGrid(grb.data)
@
...
file.name <- GribGrab(urls.out[1], pred.6hr, 
    levels, variables, verbose = FALSE)
...
file.name <- GribGrab(urls.out, pred.now, levels, variables, 
